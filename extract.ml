...
let apply_mask img mask =
  Printf.sprintf
    "convert '%s' '%s' -alpha off -compose CopyOpacity -composite temp.png"
    img mask

let crop_cmd (x, y, w, h) fn i =
  Printf.sprintf
    "convert temp.png -crop %dx%d+%d+%d +repage '%s%c.png'"
    w h x y fn (suff i)
